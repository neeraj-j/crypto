...
def convert_libri():
    ...
    with open(manifest_path, "r") as f:
        ...
        for line in tqdm(f):
            ...
            fname = os.path.join(src_root_dir, items[0])
            dst_fname = fname.replace("dev-clean", "aes")
            dst_aes = dst_fname.replace(".flac", "-aes.flac")
            if os.path.exists(dst_aes):
                continue
            feats, curr_sample_rate = sf.read(fname)
            target_size = len(feats)
            ...
            # Encrypt
            _, _, aes_sources = moo.encrypt(feats, moo.modeOfOperation["CBC"],
                                            key, moo.aes.keySize["SIZE_128"], iv)
            aes_sources = np.array(aes_sources) / 255.0 - 0.5
            collated_sources = feats / 255.0 - 0.5
            # make dst dir
            dst_dir = os.path.dirname(dst_fname)
            if not os.path.exists(dst_dir):
                os.makedirs(dst_dir)
            sf.write(dst_aes, aes_sources, curr_sample_rate)
            sf.write(dst_fname, collated_sources, curr_sample_rate)
