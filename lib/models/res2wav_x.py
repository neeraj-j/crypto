...
class BasicBlock(nn.Module):
    ...
    def __init__( self, inplanes, planes, kernel_size, stride, padding=0, bias=False):
        ...
        self.conv1 = ConvBNReLU(inplanes, planes, kernel_size,stride,padding)
        self.conv2 = ConvBN(planes, planes, kernel_size,1,padding) #convBN if relu
...
enc_blocks = {"BASIC": BasicBlock, "CBR": ConvBNReLU, "BOT":Bottleneck}
...
class Res2WavModel(nn.Module):
    def __init__(self, cfg):
        ...
        dim, k, stride = feature_dec_layers[-1]
...
class Encoder(nn.Module):
    def __init__(
        ...
        for dim, k, stride in conv_layers:
            ...
            pad=0
            self.conv_layers.append(block(self.in_d, dim, k, stride, padding=pad))
            self.in_d = dim
